SCRi.fn.par <-function(scrobj,nc=3,ni=1100,burn=100,skip=2,nz=200,theta=NA,Msigma=1,Mb=0,Msex=0,Msexsigma = 0,Xeff=NULL,Xsex=NULL, ss.prob=NULL,coord.scale=1000,area.per.pixel=1,thinstatespace=1,maxNN=20,dumprate=1000){
...
#gname <- paste("gridcheck_.csv")
#write.csv(file=gname,G)
...
## Randomize things here so that each chain has overdispersed starting values of parameters ## 
...
lp<-   loglam0 + Mb*beta.behave*prevcap - lp.sigma + beta1*Xeff + Msex*beta.sex*Xsex[indid]
repeat{
## theta must not be less than 0.5 
...
## Create a folder ##
folderName <- paste("MCDresults_",ts,"CH",ch,sep="")
dir.create(path=folderName)

## Store MCMC history ##
fname <- paste(folderName,"/MCDmcmchist_",ts,"CH",ch,".csv",sep="")
...
### Generate graphs
#Hout <- out$Sout*out$zout
#Hintensity <- sum(Hout==out$Gunscaled[,1])/nrow(out$Sout)
#logHint <- log(Hintensity)
#spatial.plot(out$Gunscaled[,2:3],logHint,add=TRUE,cx=1)
